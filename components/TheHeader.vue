	...
	<header>
						...
						<ul class="main-menu">
							<template v-for="item in menu">
								<li :key="item.ID" v-if="item.menu_item_parent == 0">
									<NuxtLink
										v-if="!isExternalAbsoluteUrl(item.url)"
		...
		<div :class="showSideMenu ? shownClasses : hiddenClasses" ref="side-menu">
			...
			<div class="inner-wrapper">
				<span class="btn-close" id="btn_sideNavClose" @click="showSideMenu = false"
					><i></i><i></i
				></span>
				<div class="container">
								...
								<div class="side-nav-menu">
									<template v-for="item in menu">
										<NuxtLink
											...
											v-if="item.menu_item_parent == 0 && !isExternalAbsoluteUrl(item.url)"
											:to="toInternalPath(item.url)"
										...
										<template v-for="subItem in menu">
											<NuxtLink
												...
												v-if="
													subItem.menu_item_parent == item.ID && !isExternalAbsoluteUrl(subItem.url)
												"
												:to="toInternalPath(subItem.url)"
												@click.native="showSideMenu = false"
												>{{ subItem.title }}</NuxtLink
...
export default {
	...
	methods: {
		...
		toInternalPath(url) {
			...
			// If it's an absolute URL but not matching backendURL, keep it as-is
			// (it should be rendered as <a>, but callers may still pass it here).
			return url;
		},
...
@for $i from 1 through 50 {
	.nav-icon .navbar-nav .nav-link:nth-child(#{$i}).nuxt-link-active ~ .menu-line,
